/**
 * sdio_it.c — SDIO 总线中断模式 (kwork 状态机 + FIFO 驱动)
 * 提供非阻塞命令发送和数据 FIFO 传输，不绑定 SD Card 协议。
 */
#include "sdio.h"
...
enum { IT_S_SETUP = 1, IT_S_CMD_DONE, IT_S_FIFO, IT_S_WAIT_END };

static void it_setup_cmd(kwork_t *w, data_ctx_t *c) {
    ...
    w->state = IT_S_CMD_DONE;
}
static void it_cmd_done(kwork_t *w, data_ctx_t *c) {
    uint32_t sta = hal_sdio_get_sta();
    if (hal_sdio_sta_has_error(sta)) { *c->result = -1; c->data->error = -1; w->state = 0; return; }
    if (sta & xSDIO_STA_CMDREND) { hal_sdio_clear_icr(xSDIO_STA_CMDREND); w->state = IT_S_FIFO; return; }
    w->state = IT_S_CMD_DONE;
}
static void it_fifo_xfer(kwork_t *w, data_ctx_t *c) {
    ...
    w->state = IT_S_WAIT_END;
}
static void it_wait_end(kwork_t *w, data_ctx_t *c) {
    uint32_t sta = hal_sdio_get_sta();
    if (hal_sdio_sta_has_error(sta)) { *c->result = -1; c->data->error = -1; w->state = 0; return; }
    if (sta & xSDIO_STA_DATAEND) { hal_sdio_clear_icr(xSDIO_STA_DATAEND); *c->result = 0; w->state = 0; return; }
    w->state = IT_S_WAIT_END;
}
static const data_step_t it_steps[] = { it_setup_cmd, it_cmd_done, it_fifo_xfer, it_wait_end };

static int cmd_work_fn(kwork_t *w) { cmd_ctx_t *c=(cmd_ctx_t*)w->ctx; cmd_steps[w->state-1](w,c); return w->state; }
static int data_work_fn(kwork_t *w){ data_ctx_t *c=(data_ctx_t*)w->ctx; it_steps[w->state-1](w,c); return w->state; }
...
void sdio_it_ioctl(Device *self, ioctl_cmd_t cmd, void *arg) {
    ...
    case SDIO_DATA_XFER:
        if (arg) {
            ...
            data_ctx_t ctx = { .dev = self, .data = (sdio_data_t*)arg, .result = &result };
...
size_t sdio_it_read(Device *self, void *buf, size_t count)  { (void)self;(void)buf;(void)count; return 0; }
void   sdio_it_write(Device *self, const void *buf, size_t count) { (void)self;(void)buf;(void)count; }
