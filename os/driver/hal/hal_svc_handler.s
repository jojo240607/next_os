...
SVC_Handler:
    ...
    /* 5. 将返回值写入任务栈的 R0 位置 */
    /*    首先跳过手动压栈的 r4-r11 (32字节)，若使用了FPU还需跳过 s16-s31 (64字节) */
    mrs     r1, psp
    add     r1, r1, #32         /* 跳过 r4-r11 */
    tst     lr, #0x10           /* bit4=0 表示使用了 FPU (扩展帧) */
    it      eq
    addeq   r1, r1, #64         /* 若使用了 FPU，再跳过 s16-s31 */
    str     r0, [r1, #0]        /* 将返回值写入自动压栈帧的 R0 位置 */
